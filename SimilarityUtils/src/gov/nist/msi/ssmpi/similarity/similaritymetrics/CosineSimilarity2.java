 ...
 * @version 1.1
 */
public final class CosineSimilarity2 extends AbstractStringMetric implements Serializable {
     ...
     */
    public CosineSimilarity2(final InterfaceTokeniser tokeniserToUse) {
    ...
    public float getSimilarity(final String string1, final String string2) {
        ...
        final Hashtable<String, Long> str1TokensFreq = new Hashtable<String, Long>();
        for (String str1 : str1Tokens) {
        	long tokenFreq = 1;
			if (str1TokensFreq.containsKey(str1)) {
				tokenFreq = str1TokensFreq.get(str1) + 1;
			}
			str1TokensFreq.put(str1, tokenFreq);
		}

        final Hashtable<String, Long> str2TokensFreq = new Hashtable<String, Long>();
        for (String str2 : str2Tokens) {
        	long tokenFreq = 1;
			if (str2TokensFreq.containsKey(str2)) {
				tokenFreq = str2TokensFreq.get(str2) + 1;
			}
			str2TokensFreq.put(str2, tokenFreq);
		}
        
        long termsInString1 = 0;
        long termsInString2 = 0;
        long commonTerms = 0;
        
        for (String str : allTokens) {
        	long freq1 = 0;
        	if (str1TokensFreq.containsKey(str)) {
        		freq1 = str1TokensFreq.get(str);
        	}
        	termsInString1 += (freq1*freq1);

        	long freq2 = 0;
        	if (str2TokensFreq.containsKey(str)) {
        		freq2 = str2TokensFreq.get(str);
        	}
        	termsInString2 += (freq2*freq2);
        	
        	commonTerms += (freq1*freq2);
        }
        
        //return CosineSimilarity
        double result = (double)commonTerms / (Math.pow((double) termsInString1, 0.5f) * Math.pow((double) termsInString2, 0.5f));
        return (float) result;
    }
